...
InformeModel.getInformeAsistencia = function (id, callback) {
    if (connection) {
        var sql = "SELECT id_asistencias, fecha, observaciones, P.nombre_1, P.nombre_2, M.nombre, C.nombre"
        +" FROM tb_asistencias AS A"
        +"      INNER JOIN tb_personas AS P ON(P.id_personas=A.id_persona)"
        +"      INNER JOIN tb_materias AS M ON(A.id_materias=M.id_materia)"
        +"      INNER JOIN ct_carreras AS C ON(M.id_carreras=C.id_carrera)"
        +"          WHERE(P.id_personas="+connection.escape(id)+" AND A.fecha BETWEEN '2021-01-08' AND '2021-02-09')"
        +"              ORDER BY A.fecha"
...
InformeModel.getInformePrestamo = function(id, callback){
    if(connection){
        var sql = "SELECT id_prestamos, fecha_prestamo, fecha_entrega, P.id_persona, A.elemento, A.estado, PE.nombre_1, PE.apellido_1"
        +" FROM tb_prestamos AS P"
        +"      INNER JOIN ct_audio_visuales AS A ON(A.id_elemento=P.id_elemento)"
        +"      INNER JOIN tb_personas AS PE ON(P.id_personas=PE.id_persona)"
