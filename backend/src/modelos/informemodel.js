...
InformeModel.postInformePrestamo = function(parametros, callback){
    if(connection){
        var sql = "SELECT id_prestamo, DATE_FORMAT(fecha_prestamo, '%y-%m-%d') as fecha_prestamo, DATE_FORMAT(fecha_entrega, '%y-%m-%d') as fecha_entrega, A.elemento, A.estado, PE.nombre_1, PE.apellido_1"
        ...
        +"          WHERE(A.id_elemento="+connection.escape(parametros.id)+" AND P.fecha_prestamo BETWEEN "+connection.escape(parametros.fechaInicio)+" AND "+connection.escape(parametros.fechaFin)+ ")"
